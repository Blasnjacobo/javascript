...
which gets assigned the return value of the callback function from the previous iteration, the second is the current element being processed, 
...
const watchList = [
  ...
  function getRating(arr) {
    ...
    let newArr = [...arr];
    let averageRating = newArr
    .filter(movie => movie.Director == "Christopher Nolan")
    .map(movie => (Number(movie.imdbRating)))
    .reduce((sum, rating) => sum + rating)/
    newArr.filter(movie => movie.Director === "Christopher Nolan").length;
    /*
    .reduce((sum, rating) => (sum + rating)/
    newArr.filter(movie => movie.Director == "Christopher Nolan").length);
    */
